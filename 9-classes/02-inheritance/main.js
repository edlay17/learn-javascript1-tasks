...
class Rabbit extends Animal {
  constructor(name) {
    super(name); // вызов родительского конструктора должен быть перед обращением к this, т.к. именно родительский консутрктор инициализиует создание this (родительский консутрктор имеет специальный флаг для этого)
...
const clock1 = new ExtendedClock({ 
  template: 'h:m:s', 
  precision: 100
})
